...
description: >-
  Primary model to fallback model to deterministic rules, with circuit breakers
  and rate limiters, so message delivery is never contingent on LLM availability
